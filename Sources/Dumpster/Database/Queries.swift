...
struct Queries {
    ...
    static func removeSubTag(parentTagId: String, childTagId: String) throws {
        try db.write { db in
            _ = try TagRelationship
                .filter(TagRelationship.Columns.parentTagId == parentTagId && TagRelationship.Columns.childTagId == childTagId)
                .deleteAll(db)
        }
    }
    ...
    static func getParentTag(childTagId: String) throws -> Tag? {
        try db.read { db in
            try Tag.fetchOne(db, sql: """
                SELECT t.* FROM tags t
                JOIN tag_relationships tr ON tr.parentTagId = t.id
                WHERE tr.childTagId = ?
                """, arguments: [childTagId])
        }
    }
